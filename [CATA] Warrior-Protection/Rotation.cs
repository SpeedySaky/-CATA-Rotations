...
public class ProtWarr : Rotation
{
        ...
        if (!target.IsDead() && (reaction != UnitReaction.Friendly && reaction != UnitReaction.Honored && reaction != UnitReaction.Revered && reaction != UnitReaction.Exalted) && !IsNPC(target))
...

            {
                ...
                if (Api.Spellbook.Cast("Charge"))
                {
                 ...
                 if (Api.Spellbook.CanCast("Charge") && (me.Auras.Contains("Warbringer", false)) && targetDistance <= 25)
    ...
    public override bool CombatPulse()
    {
        ...
        if (Api.Spellbook.CanCast("Battle Shout") && !me.Auras.Contains("Battle Shout") && !Api.Spellbook.OnCooldown("Battle Shout"))
        {
            Console.ForegroundColor = ConsoleColor.Green;
    ...
    private void LogPlayerStats()
    {
        ...
        if (me.Auras.Contains("Warbringer",false))
        {
            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine("Have Warbringer");
            Console.ResetColor();
